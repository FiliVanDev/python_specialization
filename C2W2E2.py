# Задание, оцениваемое сокурсниками: Создание иерархий классов
import pygame
import random
...
class Knot(Polyline):
    ...
    def draw_points(self, points, width=3, color=(255, 255, 255)):
        """метод отрисовки линии на экране"""
...
def draw_help():
    ...
    for i, text in enumerate(data):
        gameDisplay.blit(
            font1.render(text[0], True, (128, 128, 255)), (100, 100 + 30 * i)
        )
        gameDisplay.blit(
            font2.render(text[1], True, (128, 128, 255)), (200, 100 + 30 * i)
        )
    ...
    while working:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                polyline.add_point(
                    Vec2d(event.pos), Vec2d(random.random() * 2, random.random() * 2)
                )
                knot.add_point(
                    Vec2d(event.pos), Vec2d(random.random() * 2, random.random() * 2)
                )
